...
│                                                              │
│   ChatController ──→ ChatService (Agent 核心, 945 行)         │
│                       │                                      │
│      ┌──────────┬─────┴──────┬──────────┬─────────────┐      │
│      ▼          ▼            ▼          ▼             ▼      │
...
│                                                              │
│  Knowledge/Graph/Todo/Reminder/Conversation/Profile/Admin    │
└──────────────────────────┬──────────────────────────────────┘
                           │
┌──────────────────────────▼──────────────────────────────────┐
│  MySQL 8 │ Redis 7 │ Qdrant │ Neo4j 5 │ 阿里云 OSS            │
└─────────────────────────────────────────────────────────────┘
```
...
### 📚 RAG 知识库
- 上传 → Tika 解析 → 分片 → **bge-m3 向量化** → Qdrant 语义检索
- SHA-256 去重（按用户）+ 阿里云 OSS 存储 + 下载
...
### 环境要求
Java 21+ · MySQL 8+ · Redis 7+ · Neo4j 5+ · Qdrant · Node.js 20+（前端开发）

### 1. 启动基础设施
```bash
# MySQL(3306) + Redis(6379) + Neo4j(7687) + Qdrant(6334) 需预先运行
...
│   ├── config/           # Redis / MyBatis-Plus / Web / OSS / 限流 / 鉴权配置
│   ├── controller/       # REST 控制器（Chat/Knowledge/Graph/Todo/Reminder/...）
│   ├── service/          # 业务服务（ChatService = Agent 核心, 945 行）
...
| [md/ISSUES.md](md/ISSUES.md) | 31 条踩坑记录 + 15 条工程教训 |
...
| POST/GET/DELETE | `/api/knowledge/documents*` | 文档上传 / 列表 / 删除 / 下载 |
