    ...
    public static void main(String[] args) throws Exception {
        ...
        HttpClient.newHttpClient().sendAsync(req, HttpResponse.BodyHandlers.ofLines())
                .thenAccept(r -> {
                    r.body().filter(l -> l.startsWith("data: "))
                     ...
                     .forEach(d -> {
                        try {
                            ...
                            if (hr) { rcN[0]++; rcB.append(delta.get("reasoning_content").asText()); }
                            if (hc) { ctN[0]++; ctB.append(delta.get("content").asText()); }
                            if (ht) { tcN[0]++; tcB.append(delta.get("tool_calls").toString()); }
                    ...
                    System.out.println(tcB.toString().substring(0, Math.min(500, tcB.length())));
                })
                .join();
    }
