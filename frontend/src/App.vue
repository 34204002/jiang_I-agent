...
import { state, loadAgentConfig } from './stores/state'
...
onMounted(() => {
  ...
  if (token) { loadConversations(); loadAgentConfig() }
})
</script>
...
<div v-else class="app-shell">
  <Sidebar />
  <main class="main">
    <nav v-if="isChat()" class="tabs" role="tablist" aria-label="主导航">
      <button :class="['tab',{active:state.activeTab==='chat'}]" @click="state.activeTab='chat'"><span class="dot"></span>对话</button>
      <button :class="['tab',{active:state.activeTab==='knowledge'}]" @click="state.activeTab='knowledge'"><span class="dot"></span>知识库</button>
